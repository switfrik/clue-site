...
import { TEAM_SURVEY_QUESTIONS } from "@/lib/survey-questions";
...
export default function TeamSurveyPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data: survey } = await supabase
        .from("surveys").select("id,title,period").eq("is_active", true).eq("target", "team").single();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const missing = TEAM_SURVEY_QUESTIONS.filter(q => q.required && !answers[q.id]);
  ...
  if (!activeSurvey) return (
    <div style={{ maxWidth: 560, margin: "6rem auto", padding: "0 1.5rem", textAlign: "center" }}>
      <p style={{ fontSize: "1rem", fontWeight: 600, color: "var(--navy)" }}>현재 진행 중인 과제팀 만족도 조사가 없습니다</p>
  ...
  if (alreadySubmitted) return (
    <div style={{ maxWidth: 560, margin: "6rem auto", padding: "0 1.5rem", textAlign: "center" }}>
      ...
      <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", marginTop: 8 }}>
        [{activeSurvey.period}] 과제팀 만족도 조사에 응답해 주셔서 감사합니다.
      </p>
    </div>
  );
  ...
  if (submitted) return (
    <div style={{ maxWidth: 560, margin: "6rem auto", padding: "0 1.5rem", textAlign: "center" }}>
      ...
      <p style={{ fontSize: "0.88rem", color: "var(--text-secondary)", marginTop: "0.75rem", lineHeight: 1.7 }}>
        소중한 의견은 자문 서비스 및 전산시스템 개선에 적극 반영하겠습니다.
      </p>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "3rem 1.5rem" }}>
      <p className="section-label">과제팀 만족도 조사</p>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        {TEAM_SURVEY_QUESTIONS.map((q, idx) => (
